...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output", type=Path, help="Cartella output (riservata future release)")
    ...
    parser.add_argument("--sanitize", action="store_true", help="Crea copia conforme sanitizzata")
...
def main() -> int:
    ...
    if args.sanitize:
        output_dir, summary = sanitize(args.input_folder, profile, dry_run=args.dry_run)
    else:
    ...
    if args.json:
        ...
        for item in summary.files:
            print(f"{item.source} -> {item.status.upper()} ({len(item.issues)} issue)")
        if output_dir:
